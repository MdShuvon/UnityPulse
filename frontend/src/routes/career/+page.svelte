...
<script lang="ts">
  ...
  import { Loader2, MapPin, Briefcase, Clock, Search, FileText } from "lucide-svelte";
  ...
  <main class="main-content">
    <div class="page-hero">
      ...
      <div class="hero-content">
        <Briefcase size={36} class="hero-icon" />
        <h1 class="hero-title">ক্যারিয়ার সুযোগ</h1>
        <p class="hero-sub bangla">আপনার দক্ষতা দিয়ে সমাজে পরিবর্তন আনুন</p>
        <a href="/career/mine" class="hero-link">
          <FileText size={14} /> আমার আবেদনসমূহ
        </a>
      </div>
    </div>
      ...
      {#each filteredJobs as job}
        <div class="job-card">
          <div class="job-top">
            ...
            <span
              class="status-badge"
              class:closed={job.status === "CLOSED" || isExpired(job.deadline)}
            >
              {job.status === "CLOSED" || isExpired(job.deadline)
                ? "Closed"
                : "Open"}
            </span>
          </div>
          ...
          <div class="job-tags">
            <span class="tag"
              ><Briefcase size={12} /> {job.jobType || "Full-time"}</span
            >
            <span class="tag"
              ><MapPin size={12} /> {job.location || "Bangladesh"}</span
            >
            {#if job.experience}
          ...
          <p class="job-desc bangla">
            {job.requirements
              ? decodeHtml(job.requirements).slice(0, 150) + "..."
              : "বিস্তারিত দেখুন..."}
          </p>
          ...
          <div class="job-foot">
            <span class="deadline mono">
              {#if job.deadline}
                ⏰ {timeRemaining(job.deadline)} · {formatDeadline(
                  job.deadline,
                )}
              {:else}
                কোনো deadline নেই
    ...
    {:else}
      <div class="empty-state">
        ...
        <p class="empty-body bangla">
          নতুন সুযোগ শীঘ্রই আসছে। পরে আবার চেক করুন।
        </p>
      </div>
    {/if}
  </main>
...
<style>
  ...
  .career-page .page-hero { background: linear-gradient(135deg, #2E7A69, #1F5D50); }
