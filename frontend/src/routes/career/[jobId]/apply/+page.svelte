      ...
      if (res.ok) {
        ...
        if (job.applicationFee > 0) {
          ...
          if (paymentRes.ok && paymentData.gatewayUrl) {
            ...
            goto(
              `/career/${jobId}/apply/status?paymentId=${paymentData.paymentId}&amount=${job.applicationFee}`,
            );
          } else {
  ...
  <main class="main-content">
    ...
    {:else if job}
      <div class="apply-form-card">
        ...
        <p class="form-sub bangla">
          {job.title} · {job.department || "UnityPulse Foundation"}
        </p>
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            placeholder="কেন আপনি এই পদের জন্য উপযুক্ত..."
          ></textarea>
        </div>
        ...
        {#if job.applicationFee > 0}
          <div class="fee-box">
            <div class="fee-row">
              ...
              <span class="fee-amount mono"
                >{formatFee(job.applicationFee)}</span
              >
            </div>
            <div class="fee-note bangla">
              <Lock size={12} /> এই ফি প্ল্যাটফর্ম পরিচালনায় ব্যবহৃত হয়। ফি
              <strong>অ-ফেরতযোগ্য</strong>।
            </div>
          </div>
        {/if}
