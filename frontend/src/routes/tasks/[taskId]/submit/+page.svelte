...
<script lang="ts">
  ...
  import { Loader2, Camera, FileText, ArrowLeft, Upload, AlertCircle, CheckCircle2 } from 'lucide-svelte';
  ...
  async function fetchTaskDetail() {
    try {
      ...
      if (res.ok) {
        task = await res.json();
        if (task.mySubmission?.status === 'PENDING' || task.mySubmission?.status === 'APPROVED') {
          goto(`/tasks/${taskId}`);
        }
      } else {
      ...
      <div class="submit-form-card">
        ...
        {#if task.proofType === 'PHOTO' || task.proofType === 'BOTH'}
          <div class="form-group">
            <label class="form-label" for="photos">ছবি আপলোড করুন (সর্বোচ্চ ৫টা)</label>
        ...
        {#if task.proofType === 'TEXT' || task.proofType === 'BOTH'}
          <div class="form-group">
            ...
            <textarea class="form-input bangla" id="text-proof" rows="4" bind:value={textProof} placeholder="আপনার কাজের বর্ণনা..."></textarea>
          </div>
        {/if}
...
<style>
  ...
  .submit-page { min-height: 100vh; font-family: 'DM Sans', sans-serif; background: #F6F4EE; color: #16231F; }
  ...
  .submit-form-card { background: white; border: 1px solid #E4EDE9; border-radius: 16px; padding: 24px; }
  ...
  .photo-preview-list { display: flex; gap: 8px; margin-top: 10px; flex-wrap: wrap; }
  .photo-preview { position: relative; width: 80px; height: 80px; border-radius: 8px; overflow: hidden; }
  .photo-preview img { width: 100%; height: 100%; object-fit: cover; }
  .remove-btn { position: absolute; top: 2px; right: 2px; width: 20px; height: 20px; border-radius: 50%; background: rgba(0,0,0,0.6); color: white; border: none; cursor: pointer; font-size: 12px; line-height: 1; }
  ...
  .success-box { display: flex; align-items: center; gap: 8px; background: #E8F5E9; border: 1px solid #C8E6C9; color: #2E7D32; border-radius: 10px; padding: 12px; margin-top: 16px; font-size: 13px; font-family: 'Hind Siliguri', sans-serif; }
