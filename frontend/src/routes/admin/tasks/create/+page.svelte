<!-- frontend/src/routes/admin/tasks/create/%2Bpage.svelte -->
<script lang="ts">
...
async function fetchTaskForEdit(id: string) {
    try {
      ...
      if (res.ok) {
        ...
        pointsValue = task.pointValue || 10;
        
        // Deadline handle
        if (task.date) {
          const taskDate = new Date(task.date);
          if (taskDate.getFullYear() >= 2099) {
            noDeadline = true;
            deadline = '';
          } else {
            noDeadline = false;
...
onMount(async () => {
  ...
  try {
    ...
    if (!authRes.ok) {
      goto('/login?redirect=/admin/tasks/create');
      return;
    }
    
    const user = await authRes.json();
    
    // SUPER_ADMIN and LOCAL_ADMIN can create/edit tasks
    ...
    const id = params.get('edit') || params.get('id');
    if (id) {
      isEditMode = true;
      taskId = id;
      fetchTaskForEdit(id);
...
<div class="task-form-page">
  ...
  <p class="page-sub bangla">
    {isEditMode ? "Task তথ্য আপডেট করুন" : "Community task তৈরি করুন"}
  </p>
