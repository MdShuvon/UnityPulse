...
export async function careerRoutes(app: FastifyInstance) {
  ...
  app.post('/career/jobs/:id/apply',
    ...
    async (req, reply) => {
      ...
      const result = await careerService.applyForJob(userId, jobId, cvFile);
  ...
  // POST /career/payment/verify — (পরে মুছে ফেলা হবে, নিচের নোট দেখো)
